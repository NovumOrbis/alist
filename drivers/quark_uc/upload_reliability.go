...
// upPreReliable intentionally does not retry: replaying /file/upload/pre can allocate
// a second upload task/FID. It only adds cancellation and stage attribution.
...
func (d *QuarkOrUC) upFinishReliable(ctx context.Context, pre UpPreResp, parentID, fileName string, fileSize int64) error {
	...
	for attempt := 1; attempt <= uploadControlMaxAttempts; attempt++ {
		...
		if pre.Data.Fid != "" {
			...
			if visible {
				log.Warnf("quark upload stage=finish returned transient provider error but uploaded object fid=%s is visible; treating as success: %v", pre.Data.Fid, err)
				return nil
			}
			...
			log.Warnf("quark upload stage=finish cannot verify uploaded object name=%q size=%d because pre response has no fid; failing closed unless finish retry succeeds: %v", fileName, fileSize, err)
		}
		if attempt == uploadControlMaxAttempts {
			return wrapQuarkUploadStage("finish", fmt.Errorf("transient provider error after %d attempts: %w", attempt, err))
		}

		log.Warnf("quark upload stage=finish transient provider error attempt=%d/%d: %v; target not verified, retrying finish", attempt, uploadControlMaxAttempts, err)
		if err := waitWithContext(ctx, backoff); err != nil {
			return wrapQuarkUploadStage("finish", err)
		}
		backoff *= 2
