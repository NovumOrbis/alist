...
func e1ValidPreResponse() UpPreResp {
	var pre UpPreResp
...
func e1Wait[T any](t *testing.T, ch <-chan T, what string) T {
	t.Helper()
	select {
	case value := <-ch:
		return value
...
func TestE1HardeningPreHTTP500EmptyJSONFailsClosed(t *testing.T) {
	calls := 0
...
func TestE1HardeningPreHTTP500HTMLFailsClosed(t *testing.T) {
	calls := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestE1HardeningPreHTTP200ProviderRejectionFailsClosed(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/1/clouddrive/file/upload/pre" {
			http.NotFound(w, r)
			return
...
func TestE1HardeningPutStopsBeforeHashOnInvalidPre(t *testing.T) {
...
func TestE1HardeningPreTransportErrorUsesSingleWireAttempt(t *testing.T) {
	var mu sync.Mutex
	attempts := 0
...
func TestE1HardeningPreCallerCancellationCancelsInFlightRequest(t *testing.T) {
	...
	e1Wait(t, started, "PRE RoundTrip start")
	cancel()
	err := e1Wait(t, done, "canceled PRE return")
...
func TestE1HardeningPre307IsRejectedWithoutReplay(t *testing.T) {
	var mu sync.Mutex
...
func TestE1HardeningPre302IsRejectedWithoutMethodChange(t *testing.T) {
	var mu sync.Mutex
	methods := make([]string, 0, 2)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		mu.Lock()
...
func TestE1HardeningPreCrossOriginRedirectIsNotFollowed(t *testing.T) {
	redirectedCalls := 0
	redirected := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		redirectedCalls++
		writeJSON(w, http.StatusOK, e1ValidPreResponse())
	}))
...
func TestE1HardeningPreProviderErrorIsReportedForQuarkAndUC(t *testing.T) {
	cases := []struct {
		name    string
		referer string
		pr      string
...
func TestE1HardeningPreCanceledBeforeStartMakesNoRequest(t *testing.T) {
	calls := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		writeJSON(w, http.StatusOK, e1ValidPreResponse())
...
func TestE1HardeningValidPreContinuesExistingTask(t *testing.T) {
	calls := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		writeJSON(w, http.StatusOK, e1ValidPreResponse())
...
func TestE1HardeningPreRequiresExactHTTP200(t *testing.T) {
	for _, status := range []int{http.StatusCreated, http.StatusNoContent} {
		t.Run(http.StatusText(status), func(t *testing.T) {
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if status == http.StatusCreated {
					writeJSON(w, status, e1ValidPreResponse())
					return
				}
...
func TestE1HardeningPreStructuralFieldsFailClosed(t *testing.T) {
	basePre := e1ValidPreResponse()
	tests := []struct {
		name   string
...
func TestE1HardeningPreUploadURLShapeMatchesBaselineTargetUse(t *testing.T) {
	valid := []string{
		"http://host",
		"http://host:443",
		"abcd://host",
...
type e1ErrorReader struct {
	data []byte
	err  error
}

func (r *e1ErrorReader) Read(p []byte) (int, error) {
	if len(r.data) > 0 {
		n := copy(p, r.data)
		r.data = r.data[n:]
		return n, nil
...
func (r *e1ErrorReader) Close() error {
	return nil
}

func TestE1HardeningPrePartialBodyReadErrorIsNotRetried(t *testing.T) {
	attempts := 0
	client := base.NewRestyClient()
	client.SetTransport(roundTripFunc(func(req *http.Request) (*http.Response, error) {
		attempts++
...
func TestE1HardeningPreMergesResponseSessionCookie(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestE1HardeningPreDoesNotMutateSelectedRestyClient(t *testing.T) {
	var redirected int
